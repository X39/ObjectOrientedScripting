...
public class Scanner {
	...
	static Scanner() {
		...
		for (int i = 97; i <= 99; ++i) start[i] = 7;
		for (int i = 101; i <= 122; ++i) start[i] = 7;
		start[45] = 32; 
		start[34] = 4; 
		start[58] = 33; 
		start[59] = 9; 
		...
		start[124] = 22; 
		start[61] = 23; 
		for (int i = 42; i <= 42; ++i) start[i] = 10;
		for (int i = 47; i <= 47; ++i) start[i] = 10;
		start[62] = 24; 
		start[60] = 25; 
		start[40] = 11; 
		start[41] = 12; 
	...
	void CheckLiteral() {
		switch (t.val) {
			...
			case "scalar": t.kind = 22; break;
			case "int": t.kind = 23; break;
			case "double": t.kind = 24; break;
			case "float": t.kind = 25; break;
			case "bool": t.kind = 26; break;
			case "boolean": t.kind = 27; break;
			case "string": t.kind = 28; break;
			case "object": t.kind = 29; break;
			case "true": t.kind = 30; break;
			case "false": t.kind = 31; break;
			case "static": t.kind = 35; break;
			case "namespace": t.kind = 36; break;
			case "native": t.kind = 37; break;
			case "assign": t.kind = 38; break;
			case "simple": t.kind = 39; break;
			case "endAssign": t.kind = 40; break;
			case "fnc": t.kind = 41; break;
			case "void": t.kind = 42; break;
			case "strict": t.kind = 43; break;
			case "endFnc": t.kind = 44; break;
			case "operator": t.kind = 45; break;
			case "endOperator": t.kind = 46; break;
			case "class": t.kind = 47; break;
			case "interface": t.kind = 49; break;
			case "override": t.kind = 50; break;
			case "auto": t.kind = 51; break;
			case "for": t.kind = 52; break;
			case "while": t.kind = 53; break;
			...
			case "is": t.kind = 60; break;
			case "throw": t.kind = 61; break;
			case "return": t.kind = 62; break;
			case "switch": t.kind = 63; break;
			case "case": t.kind = 64; break;
			case "default": t.kind = 65; break;
			case "SQF": t.kind = 67; break;
			default: break;
		}
	}
	...
	Token NextToken() {
		...
		switch (state) {
			...
			case 35:
				recEnd = pos; recKind = 3;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'd' || ch >= 'f' && ch <= 'z') {AddCh(); goto case 7;}
				else if (ch == ':') {AddCh(); goto case 8;}
				else if (ch == 'e') {AddCh(); goto case 36;}
				else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 36:
				recEnd = pos; recKind = 3;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'e' || ch >= 'g' && ch <= 'z') {AddCh(); goto case 7;}
				else if (ch == ':') {AddCh(); goto case 8;}
				else if (ch == 'f') {AddCh(); goto case 37;}
				else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 37:
				recEnd = pos; recKind = 3;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'b' && ch <= 'z') {AddCh(); goto case 7;}
				else if (ch == ':') {AddCh(); goto case 8;}
				else if (ch == 'a') {AddCh(); goto case 38;}
				else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 38:
				recEnd = pos; recKind = 3;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 't' || ch >= 'v' && ch <= 'z') {AddCh(); goto case 7;}
				else if (ch == ':') {AddCh(); goto case 8;}
				else if (ch == 'u') {AddCh(); goto case 39;}
				else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 39:
				recEnd = pos; recKind = 3;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 'k' || ch >= 'm' && ch <= 'z') {AddCh(); goto case 7;}
				else if (ch == ':') {AddCh(); goto case 8;}
				else if (ch == 'l') {AddCh(); goto case 40;}
				else {t.kind = 3; t.val = new String(tval, 0, tlen); CheckLiteral(); return t;}
			case 40:
				recEnd = pos; recKind = 3;
				if (ch >= '0' && ch <= '9' || ch >= 'A' && ch <= 'Z' || ch == '_' || ch >= 'a' && ch <= 's' || ch >= 'u' && ch <= 'z') {AddCh(); goto case 7;}
				else if (ch == ':') {AddCh(); goto case 8;}
